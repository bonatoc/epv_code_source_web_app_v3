...
.controller('ProductsCtrl', [
	...
	function(
	...
	) 
		{
			...
			// DO NOT DELETE sayHi. JUST TO DEMNONSTRATE THAT DB STRINGS MAY NOW CONTAIN DIRECTIVES, AS THEY ARE COMPILED
			// $scope.sayHi = function(string){
			// 	string = $filter('translate')(string);
			// 	alert('string : '+string);
			// }
			// - EOF - DO NOT DELETE sayHi. JUST TO DEMNONSTRATE THAT DB STRINGS MAY NOW CONTAIN DIRECTIVES, AS THEY ARE COMPILED
			...
			// $scope.Products = appDataFactory.getObject('Products');
				// INJECT STRINGS NAMES INTO OBJECTS, SO ANGULAR TRANSLATE CAN "CATCH" THEM 
				// $scope.Products = stringsFactory.retrieveAndInjectStringNames($scope.Products, mainarrayData.Strings);
				// TODO: BUILD A getObjectByID FUNCTION IN THE appDataFactory
				
				$scope.Product = appDataFactory.getObjectByID('Products',selectedProductID);
				
				console.log('$scope.Product BEFORE STRINGS ->');
				console.log($scope.Product);
				
				// TODO : MOVE THIS LOGIC TO stringsFactory.retrieveAndInjectStringNames : EVALUATE IF A SINGLE OBJECT IS PASSED, OR A AN ARRAY-LIKE OBJECT ( — WHAT? :-) )
				var empty_array = [];
				...
				console.log('$scope.Product AFTER STRINGS ->');
				console.log($scope.Product);
				...
				// GET THE PRODUCT PRICES
					...
					// INJECT STRINGS IDENTIFIERS (TODO: INVESTIGATE, THERE MUST BE A BETTER WAY, AT A LOWER LEVEL)
		...
		// $scope.$watch(function($scope) { return $scope.selectedProductTabsIndex },
		//     function(newValue, oldValue) {
		// 		alert('oldValue : '+oldValue+' has changed and is now : '+newValue);
		//     }
		// ); // $scope.$watch(function($scope) { return $scope.selectedTopnavIndex
			...
			// 	  			$scope.setSelectedProductTabsIndex(2);
			// }
